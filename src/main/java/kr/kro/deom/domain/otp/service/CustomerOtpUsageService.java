...
public class CustomerOtpUsageService {
    ...
    public List<OtpUsageResponse> getUsagesByStatus(Long userId, List<OtpStatus> status) {
        return mapToResponse(otpRepository.findByUserIdAndStatusInOrderByCreatedAtDesc(userId, status));
    }
    ...
    private List<OtpUsageResponse> mapToResponse(List<OtpUsage> usages) {
        return usages.stream().map(usage -> {
            ...
            OtpUsageResponse.OtpUsageResponseBuilder builder = OtpUsageResponse.builder()
                    .otpId(usage.getId())
                    .storeId(store.getId())
            ...
            if (usage.getType() == OtpType.DEOM && usage.getDeomId() != null) {
                ...
                builder.deomId(deom.getId())
                        .deomName(deom.getName());
            }
