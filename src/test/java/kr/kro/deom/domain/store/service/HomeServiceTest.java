      ...
      when(deomRepository.findByStoreIdOrderByRequiredStampAmountAsc(anyLong()))
          .thenAnswer(
              invocation -> {
                ...
                return deoms.stream().filter(deom -> deom.getStoreId().equals(storeId)).toList();
              });
  ...
  class GetAvailableStoresTest {
    ...
    void getAvailableStores_ReturnsOnlyStoresWithAvailableDeoms() {
      ...
      try (MockedStatic<SecurityUtils> securityUtils = mockStatic(SecurityUtils.class)) {
        ...
        assertThat(deomInfos.get(0).getStatus()).isEqualTo(DeomStatus.AVAILABLE); // 아메리카노(10) 가능
        assertThat(deomInfos.get(1).getStatus()).isEqualTo(DeomStatus.AVAILABLE); // 라떼(15) 가능

        // 두 번째 스토어의 덤 상태 확인
        deomInfos = result.get(1).getDeoms();
        assertThat(deomInfos).hasSize(1);
        assertThat(deomInfos.get(0).getStatus()).isEqualTo(DeomStatus.AVAILABLE); // 파스타(12) 가능
      }
    }
