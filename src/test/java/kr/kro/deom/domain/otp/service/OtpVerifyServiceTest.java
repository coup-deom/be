...
class OtpVerifyServiceTest {
  ...
  @InjectMocks private OtpVerifyService otpVerifyService;
  ...
  void verifyOtpFromRedisSuccessfully() {
    ...
    try (MockedStatic<SecurityUtils> securityUtils = Mockito.mockStatic(SecurityUtils.class)) {
      ...
      // When
      OtpVerifyResponse response = otpVerifyService.otpVerify(testOtpCode);
  ...
  void invalidOtpThrowsException() {
    ...
    when(hashOperations.entries(anyString())).thenReturn(new HashMap<>());
    when(otpUsageRepository.findIdAndTypeAndStatusByOtp(testOtpCode)).thenReturn(Optional.empty());

    // When & Then
    OtpException exception =
        assertThrows(
            OtpException.class,
            () -> otpVerifyService.otpVerify(testOtpCode));
