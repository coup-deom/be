...
class OtpOwnerVerifyServiceTest {
    ...
    @BeforeEach
    void setup() {
        // lenient mode로 설정하여 불필요한 stubbing 경고 방지
    ...
    void verifyOtpFromRedisSuccessfully() {
        ...
        when(otpRepository.findIDByOtpAndStoreIdAndStatus(
                        testOtpCode, testStoreId, OtpStatus.PENDING))
        ...
        when(otpRepository.save(any(OtpUsage.class))).thenReturn(savedOtpUsage);
    ...
    void verifyExistingOtpFromRedisSuccessfully() {
        ...
        when(otpRepository.findIDByOtpAndStoreIdAndStatus(
                        testOtpCode, testStoreId, OtpStatus.PENDING))
                .thenReturn(Optional.of(testOtpUsageId));
        when(storeRepository.findOwnerIdByStoreId(testStoreId)).thenReturn(testUserId);
    ...
    void verifyOtpFromDatabaseSuccessfully() {
        ...
        // 현재 로그인한 사용자 ID로 상점 ID 조회
        when(storeRepository.findIdByOwnerId(testUserId)).thenReturn(testStoreId);

        when(otpRepository.findOtpInfoByOtpAndStoreIdAndStatus(
                        testOtpCode, testStoreId, OtpStatus.PENDING))
